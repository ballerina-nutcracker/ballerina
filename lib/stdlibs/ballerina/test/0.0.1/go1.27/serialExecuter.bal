...
function executeTest(TestFunction testFunction) {
    ...
    foreach string 'group in testFunction.groups {
        groupStatusRegistry.incrementExecutedTest('group);
    }
...
function executeDataDrivenTestSet(TestFunction testFunction) {
    ...
    while keys.length() != 0 {
        string key = keys[0];
        AnyOrError[] value = values[0];
        keys = sliceFrom(keys, 1);
        values = sliceFromAnyOrErrorArrays(values, 1);
...
function executeTestFunction(TestFunction testFunction, string suffix, TestType testType,
    ...
    any|error output;
    if params == () {
        output = trap invokeFunction(testFunction.executableFunction, []);
    } else {
        output = trap invokeFunction(testFunction.executableFunction, params);
    }
...
function executeFunction(TestFunction|function testFunction) returns ExecutionError? {
    function toInvoke;
    string invokedName = "";
    if testFunction is function {
        toInvoke = testFunction;
    } else {
        toInvoke = testFunction.executableFunction;
        invokedName = testFunction.name;
    }
    any|error output = trap invokeFunction(toInvoke, []);
    if output is error {
        enableExit();
        return error ExecutionError(invokedName + "\n" + getErrorMessage(output));
    }
}
