...
// GetSyntaxTreeWithIdentity parses content into a syntax tree whose diagnostics are
// registered under diagnosticIdentity, which may differ from the sourcePath shown to the
// user, and reports the resulting syntax diagnostics to ctx.
...
// getSyntaxTree registers content under diagnosticIdentity, parses it into a syntax tree
// and, when reportDiagnostics is set, reports each syntax diagnostic to ctx.
func getSyntaxTree(
	ctx *context.CompilerContext,
	sourcePath string,
	diagnosticIdentity string,
