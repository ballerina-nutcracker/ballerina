...
// printDiagnostics writes every diagnostic in diagResult to w.
...
// printDiagnostic writes a single diagnostic to w as a header line followed by, when
// the diagnostic has a location with source text, the file reference and an underlined
// source snippet.
...
// printSourceSnippetContent writes the source lines spanned by loc, each followed by a
// caret line in severityColor underlining the diagnostic's columns.
