...
// runCompilation holds everything `bal run` needs after the driver pipeline
// has finished: the compilation context, the resolver that produced the
// dependency sources, the generated BIR packages and, on success, the runtime.
// loadFailed marks the case where the package could not even be parsed.
...
// compileRunInput compiles the `bal run` input (a single .bal file, a package
// directory or a workspace member) straight through the driver pipeline,
// replacing the older path that loaded a projects.Project first. It installs
// lifecycle hooks that collect the parser/AST/CFG/BIR dumps requested by
// buildOptions, writes them out once the pipeline finishes, and — unless
// already running in native mode — re-executes through the native projects
// pulled in by the resolved dependencies.
...
// add appends one dump entry to target under the collector's lock, since the
// driver invokes the lifecycle hooks from concurrent per-module goroutines.
...
// flushParser writes the collected parser debug output to w in deterministic
// module order. It is a no-op when w is nil, i.e. when no parser dump was asked for.
...
// flush writes the collected AST dumps verbatim to w, followed by the CFG and
// BIR dumps wrapped in the BEGIN/END banners the corpus goldens expect.
...
// orderedRunDumps returns a copy of values sorted by module name and then
// source path, so dump output does not depend on the order the concurrent
// lifecycle hooks happened to fire in.
...
// moduleDescriptors returns the module IDs of every module in parsed, in
// package order.
