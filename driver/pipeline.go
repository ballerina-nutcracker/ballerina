...
// packageRoot returns the directory used as the source root for the input: the input path itself
// when it names a package directory, otherwise the directory containing the single source file.
...
// findSources locates the sources to compile and sets p.sourceRoot. For a single file, or when no
// package directory name is configured, it discovers a standalone package. Otherwise it looks for
// a workspace, selects the member matching the input path (the first member for "."), and wraps
// p.resolver in a workspace resolver so sibling members resolve locally.
...
// stopped reports whether a lifecycle hook has requested that compilation stop. Run and its
// per-module goroutines check it at each checkpoint and abandon remaining work when it is set.
func (p *Pipeline) stopped() bool { return p.stop.Load() }

// fail records err as the operational error that stopped the pipeline, keeping the first such
// error, and reports it to the OnError hook attributed to stage. A nil err is ignored.
...
// diagnosticCount returns the number of diagnostics currently recorded on the driver context,
// for use as a cursor into that append-only log.
...
// reportDiagnosticsSince reports error and fatal diagnostics recorded on the driver context since
// the cursor start, which is a diagnosticCount snapshot taken before the stage ran. Each
// diagnostic is delivered to OnError at most once across all calls, tracked by ordinal, and is
// attributed to its own recorded stage or to fallback when it has none.
...
// sourcePath resolves the source path of a module-scoped diagnostic by looking up its module and
// document index in the parsed package, returning "" when the document cannot be identified.
...
// emitParse delivers an OnParse event for every regular and test document of parsed, omitting any
// document in skip so documents already reported are not emitted twice. A hook returning true
// sets the stop flag.
...
// emitRecoveredAST builds a best-effort AST for each parsed module and emits its documents as
// recovered OnAST events, so consumers still see a tree after parsing produced errors.
...
// emitAST emits OnAST events for the documents of module, ignoring a nil module.
...
// emitASTBase emits one OnAST event per non-nil document of module, tagged with recovered.
// A hook returning true sets the stop flag.
...
// parsedDocumentSet returns the set of document pointers in parsed, used to skip re-emitting the
// root package's documents after dependency resolution extends the parsed package.
...
// pipelineStage maps a compiler context stage onto its driver PipelineStage, returning "" for a
// stage with no driver-level equivalent.
