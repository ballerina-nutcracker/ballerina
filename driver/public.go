...
// ResolvePublicNodes runs symbol resolution and top-level type resolution over
// an AST module, then publishes the module's exported symbol space into the
// shared Env (also registering it under its implicit name, except lang.runtime)
// so dependent modules can resolve against it. It returns nil if ctx is
// cancelled or either stage reports errors; diagnostics are recorded on ctx.
