...
// toAST runs the AST-build stage over every document of module concurrently,
// combining the resulting compilation units into one package node. When
// recovered is true the recovering node builder is used. It returns nil after
// draining the stage's diagnostics into ctx on cancellation or error.
func toAST(ctx *Context, module *ParsedModule, recovered bool) *moduleBase {
	if err := ctx.Err(); err != nil {
		return nil
	}
	stageCX := ctx.newCompilerContext(module.ID)
	stageCX.StartStage(compilercontext.StageASTBuild)
	packageID := modelPackageID(stageCX, module.ID)
	documents := make([]*ASTDocument, len(module.Documents))
	compilationUnits := make([]*ast.BLangCompilationUnit, len(module.Documents))
	documentContexts := make([]*compilercontext.CompilerContext, len(module.Documents))
	var wg sync.WaitGroup
	for index, document := range module.Documents {
		documentCX := ctx.newCompilerContext(module.ID)
		documentContexts[index] = documentCX
		wg.Add(1)
		go func() {
			defer wg.Done()
			if ctx.Err() != nil {
			...
			if recovered {
				compilationUnit = nodebuilder.GetRecoveredCompilationUnitForSource(documentCX, document.SyntaxTree, packageID)
			} else {
				compilationUnit = nodebuilder.GetCompilationUnitForSource(documentCX, document.SyntaxTree, packageID)
			}
	...
	hasErrors := false
	for index, documentCX := range documentContexts {
		ctx.drainModule(documentCX, module.ID, compilercontext.StageASTBuild, false, index)
		hasErrors = hasErrors || documentCX.HasErrors()
	}
	if hasErrors || ctx.Err() != nil {
		stageCX.EndStage()
		ctx.drainModule(stageCX, module.ID, compilercontext.StageASTBuild, false, 0)
		return nil
	}
	packageNode := nodebuilder.ToPackageFromCompilationUnits(stageCX, compilationUnits)
	packageNode.PackageID = packageID
	stageCX.EndStage()
	ctx.drainModule(stageCX, module.ID, compilercontext.StageASTBuild, false, 0)
	if stageCX.HasErrors() || ctx.Err() != nil {
		return nil
	}
