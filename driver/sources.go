...
// Dependency is a package dependency declared in Ballerina.toml, optionally naming the repository
// it must be resolved from.
...
// ResolutionOptions carries the build options that affect dependency resolution.
type ResolutionOptions struct {
	Offline bool
	Sticky  bool
}

// Manifest holds the parts of a package's Ballerina.toml that the driver acts on.
type Manifest struct {
	Dependencies []Dependency
	Resolution   ResolutionOptions
}

// PackageSources describes a discovered package: its descriptor, its manifest and its modules,
// with the default module always present.
...
// ModuleSources lists the source paths of one module, relative to the package root, separated
// into regular and test documents.
...
// WorkspaceSources describes a discovered workspace: its member packages in declaration order
// and the resolution options taken from the first member that loaded successfully.
type WorkspaceSources struct {
	Members    []*WorkspaceMemberSources
	Resolution ResolutionOptions
}

// WorkspaceMemberSources pairs a workspace member's root directory with its discovered sources.
type WorkspaceMemberSources struct {
	Root    string
	Sources *PackageSources
}

// Member returns the workspace member whose root matches the cleaned root path. It reports false
// for a nil receiver or an unknown root.
...
// FindWorkspaceSources discovers the packages of the workspace declared in the Ballerina.toml at
// workspaceRoot. It returns (nil, nil) when that file has no workspace table, meaning the
// directory is not a workspace. Malformed entries and members that fail to load are reported as
// diagnostics on ctx and skipped rather than returned as errors, so an error result means
// cancellation or an invalid argument.
...
// FindSources discovers the sources of the package at inputPath. A single .bal file becomes an
// anonymous single-module package; a directory is read as a Ballerina package whose descriptor
// and manifest come from its Ballerina.toml, with packageDirName supplying the default name.
// Manifest problems, including an invalid version or dependency, are reported as diagnostics on
// ctx and defaulted rather than returned as errors.
...
// dependencyFromTOML reads one [[dependency]] table, requiring non-empty org, name and a valid
// version; repository is optional.
...
// discoverModules returns the default module, built from the .bal files at root and its tests
// directory, followed by one module per subdirectory of modules/ in name order, each named
// "<package>.<directory>". A missing modules/ directory yields just the default module.
...
// discoverOptionalBalFiles behaves like discoverBalFiles but treats a missing directory, or a
// non-directory at that path, as no sources instead of an error.
...
// discoverBalFiles returns the sorted paths, relative to root, of the .bal files directly in the
// relative directory, ignoring subdirectories. It checks ctx for cancellation before and after
// reading the directory, and requires the directory to exist.
...
// convertTOMLDiagnostics adapts TOML parser diagnostics to compiler diagnostics located at the
// start of the Ballerina.toml file, preserving message and severity.
...
// manifestDiagnostic returns an error diagnostic for message located at the start of the
// package's Ballerina.toml file.
