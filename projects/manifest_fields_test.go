...
// TestManifest_IconReadmeExport parses testdata/manifest-fields-project's
// Ballerina.toml (icon, readme, [[package.modules]] export entries) and
// asserts every field lands on the in-memory PackageManifest.
func TestManifest_IconReadmeExport(t *testing.T) {
	t.Parallel()
...
// TestPack_ManifestFieldsDocs drives EmitBala end-to-end against
// testdata/manifest-fields-project and asserts the readme/icon/module-readme
// docs land at their expected docs/ locations in the archive, and that the
// icon/readme/exported-modules fields round-trip correctly when the packed
// bala is reloaded.
func TestPack_ManifestFieldsDocs(t *testing.T) {
	t.Parallel()
	source, err := filepath.Abs(filepath.Join("testdata", "manifest-fields-project"))
	if err != nil {
		t.Fatalf("abs source: %v", err)
...
// TestManifest_WorkspaceMemberReadmeNotDoubled reproduces a bug where
// defaultReadme/defaultModuleReadme stored a manifest path already prefixed
// with the workspace member's own project-relative directory (e.g.
// "pkg-a/README.md" instead of "README.md") — every later reader
// (addBalaDoc, etc.) re-joins Manifest().Readme() with the package's own
// root, so a workspace member (whose root isn't ".") ended up with that
// prefix doubled and the README/module-README failed to load.
func TestManifest_WorkspaceMemberReadmeNotDoubled(t *testing.T) {
	t.Parallel()
	...
	members := ws.Projects()
	if len(members) != 1 {
		t.Fatalf("expected 1 workspace member, got %d", len(members))
	}
	manifest := members[0].CurrentPackage().Manifest()

	if got, want := manifest.Readme(), "README.md"; got != want {
		t.Errorf("pkg-a Readme() = %q, want %q (relative to pkg-a's own root, not doubled)", got, want)
	}

	found := false
	for _, mod := range manifest.Modules() {
		if mod.Name() != "pkga.extra" {
			continue
		}
		found = true
		if got, want := mod.Readme(), filepath.ToSlash(filepath.Join("modules", "extra", "README.md")); got != want {
			t.Errorf("pkga.extra module Readme() = %q, want %q", got, want)
		}
	}
	if !found {
		t.Fatal("expected an auto-discovered 'pkga.extra' module")
	}
}
