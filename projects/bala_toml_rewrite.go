...
// Table headers and readme/icon lines allow a trailing "# comment" (valid
// TOML), and string values may be either double- or single-quoted (TOML
// basic vs. literal strings) — all four regexes below tolerate both so a
// Ballerina.toml written in either style still gets its readme/icon lines
// found and rewritten instead of a second, duplicate line being inserted.
var (
	...
	for i, line := range lines {
		...
		case sectionModule:
			if m := balaTomlNameLineRe.FindStringSubmatch(line); m != nil {
				if m[1] != "" {
					cur.name = m[1] // double-quoted
				} else {
					cur.name = m[2] // single-quoted
				}
			}
			if balaTomlReadmeLineRe.MatchString(line) {
				cur.readmeLine = i
