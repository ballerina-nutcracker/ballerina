...
The hook requires Python 3 and validates commit subjects, license headers on new Go and Ballerina files, and Go formatting, then builds and lints the workspace in parallel. Merge, `fixup!`, and `squash!` commit subjects are exempt from local message validation, but still run the remaining checks. Set `SKIP_COMMIT_HOOKS` to bypass all checks for a single commit:
