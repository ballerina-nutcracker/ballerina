...
The hook validates commit subjects, license headers on new Go and Ballerina files, and Go formatting, then builds and lints the workspace in parallel. Merge and `fixup!` commit subjects are exempt from message validation, but still run the remaining checks. Set `SKIP_COMMIT_HOOKS` to bypass all checks for a single commit:
