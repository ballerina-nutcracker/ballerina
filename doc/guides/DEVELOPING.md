...
# one span per definition inside each phase, not just the phases
...
million, and a 160 MB document that no longer reads by eye. Reach for `--nested`
when a phase is slow and the question is which definitions made it slow.
