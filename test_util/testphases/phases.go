...
// Phase names the compilation stage a test pipeline run should stop after.
type Phase int
...
// PipelineResult carries the artifacts a pipeline run produced up to its
// requested phase; later fields stay nil when the run stopped earlier or hit
// errors.
...
// LoadLanglibs returns an empty langlib symbol table: the driver-based pipeline
// resolves lang libraries as ordinary dependencies, so this exists only to keep
// the langlib argument in the pipeline entry points.
...
// RunPipeline compiles the file at inputPath from the real file system up to
// phase, using its directory as the pipeline root.
...
// RunPipelineWithContent is RunPipeline for sources that are not on disk: the
// content is compiled from an in-memory file system under inputPath's base
// name, so the path only supplies the file name used in diagnostics.
...
// runPipeline drives the driver stages over fsys up to phase, mirroring the
// driver diagnostics into the legacy compiler context at every exit point and
// returning the artifacts gathered so far. It forces the anonymous "." package
// identity that the stage corpus goldens were recorded with.
...
// restoreExplicitImports prepends the import nodes written in the source
// documents to the package's import list, so golden output shows the
// user-written imports ahead of any the pipeline added.
...
// rootModuleIndex returns the index of the root module of parsed, falling back
// to the last module when no module matches the root descriptor.
...
// populateASTResult copies an AST module's package node, and the compilation
// unit of its first document, into result.
...
// populatePublicResult copies a partially resolved module's package node, and
// the compilation unit of its first document, into result.
...
// mirrorDiagnostics replays driver diagnostics onto the legacy compiler context
// as internal or semantic errors, so tests asserting on the old context keep
// working; warnings and lesser severities are dropped.
