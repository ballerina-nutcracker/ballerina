...
func loadBuiltinPublicSymbols(env *context.CompilerEnvironment) map[semantics.PackageIdentifier]model.ExportedSymbolSpace {
	...
	for _, entry := range builtinStdlibs {
		...
		pkg := nodebuilder.ToPackageFromCompilationUnitsWithContext(cx, compilationUnits)
		if cx.HasErrors() {
			continue
		}
		pkg.PackageID = pkgID
...
func RunPipelineWithContent(env *context.CompilerEnvironment, cx *context.CompilerContext, langlibs *langlib.Symbols, phase Phase, inputPath string, content string) (*PipelineResult, error) {
	...
	if phase == PhaseAST {
		result.Package = nodebuilder.ToPackageFromCompilationUnitsWithContext(cx, []*ast.BLangCompilationUnit{result.CompilationUnit})
		return result, nil
	}
	...
	result.Package = nodebuilder.ToPackageFromCompilationUnitsWithContext(cx, compilationUnits)
	...
	birPkg, ok := birgen.GenBir(cx, result.Package)
	if !ok {
		return result, nil
	}
	result.BIRPackage = birPkg
	return result, nil
}
