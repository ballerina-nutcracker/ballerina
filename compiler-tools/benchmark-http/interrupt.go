...
func (c *cleanups) add(fn func()) {
	c.push(newCleanupStep(fn))
}

// addScoped registers a step for a resource that is torn down before the
// program ends, and returns the function that tears it down: it runs the step
// once and drops it from the stack. Until then an interrupt tears the resource
// down as part of the stack.
func (c *cleanups) addScoped(fn func()) (teardown func()) {
...
func (c *cleanups) push(step *cleanupStep) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.steps = append(c.steps, step)
}

func (c *cleanups) remove(step *cleanupStep) {
	c.mu.Lock()
	defer c.mu.Unlock()
