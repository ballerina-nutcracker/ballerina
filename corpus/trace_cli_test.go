...
var (
	...
	birGenFailureFixture         = filepath.Join("corpus", "cli", "testdata", "run", "trace", "birgen-failure")
	birGenFailureResourceFixture = filepath.Join("corpus", "cli", "testdata", "run", "trace", "birgen-failure-resource")
)
...
func TestBalRunTraceChildSpans(t *testing.T) {
	...
	t.Run("a lambda signature nests under the body that resolves it", func(t *testing.T) {
		assertSpanParent(t, events, "Local Type Resolution", "Function Signature $anonFunc$_0",
			"Function Body scaled")
	})
...
const (
	birGenFailurePackage         = "testorg/cli_trace_birgen_failure:0.1.0"
	birGenFailureResourcePackage = "testorg/cli_trace_birgen_failure_resource:0.1.0"
)

// TestBalRunTraceChildSpansOnBirGenerationFailure covers a definition whose body
// cannot be generated: its span is started before the transformation and must
// still be completed when the transformation gives up.
func TestBalRunTraceChildSpansOnBirGenerationFailure(t *testing.T) {
	t.Parallel()
	skipTraceTestOnWasm(t)
	tests := []struct {
		name    string
		...
		{"method", birGenFailureFixture, birGenFailurePackage, "Method add"},
		{"resource method", birGenFailureResourceFixture, birGenFailureResourcePackage, "Resource Method get"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			assertFailedDefinitionSpanRecorded(t, test.fixture, test.pkg, test.span)
		})
	}
}

func assertFailedDefinitionSpanRecorded(t *testing.T, fixture, pkg, spanName string) {
	t.Helper()
	balBin, repoRoot, coverDir := integrationTestBalCLI(t, true)
	workDir := copyTraceFixture(t, repoRoot, fixture)

	stdout, stderr, exitCode := runBalInDir(t, balBin, workDir, coverDir, "run", ".", "--trace", "--nested")
	if exitCode == 0 {
		t.Fatalf("expected BIR generation to fail:\nstdout:\n%s\nstderr:\n%s", stdout, stderr)
	}

	events := readTrace(t, filepath.Join(workDir, traceFileName))
	assertTraceTreeInvariants(t, events)
	recorded := phaseSpanNames(t, events, "BIR Generation", pkg)
	if !slices.Contains(recorded, spanName) {
		t.Errorf("the %s span that failed to generate is missing; recorded:\n%s",
			spanName, strings.Join(recorded, "\n"))
	}
}
