...
function testXmlIndexedAndFilterStepExtend() {
    ...
    assertError(trap x1/*.<name>[-1]);
    assertError(trap x1/*.<name|iname>[k].<iname>[-1]);
    ...
    assertError(trap x1/<price>[-1].<count>);
...
function testXmlMethodCallStepExtend() returns error? {
    ...
    xml objectFieldJ = x1/<name>[0];
    xml objectFieldK = x1/**/<brand>.map(y => y.elementChildren().get(0));
    xml objectFieldL = x1/<count>.<count>;
    assert(objectFieldJ, xml `<name>T-shirt</name><name>Backpack</name><name>Watch</name>`);
    assert(objectFieldK, xml `<name>nike</name><name>adidas</name><name>samsung</name>`);
    assert(objectFieldL, xml `<count>1</count><count>3</count><count>2</count>`);
    ...
    [xml, xml, xml] funcResult = xmlFunc();
    assert(funcResult[0], xml `<name>nike</name><name>adidas</name><name>samsung</name>`);
    assert(funcResult[1], xml `<brand><name>nike</name><local>no</local></brand><brand><name>adidas</name><local>yes</local></brand><brand><name>samsung</name><local>no</local></brand>`);
    assert(funcResult[2], xml `<count>1</count><count>3</count><count>2</count>`);
...
function assertError(any|error value) {
    if value is error {
        if (value.message() != "XML index out of range") {
            panic error("Unexpected error: " + value.message());
        }
        return;
    }
    panic error("Expected an error value");
}

public function main() returns error? {
    var single = testXMLNavigationOnSingleElement();
    foreach int index in 0...4 {
        assert(single[index], xml `<child attr="attr-val"/>`);
    }
    io:println("single-element ok"); // @output single-element ok

    var namespaced = testXMLNavigationOnSingleElementWithNamespaces();
    _ = namespaced;
    io:println("namespaced-single ok"); // @output namespaced-single ok

    var defaultNS = testXMLNavigationOnSingleElementReferToDefaultNS();
    _ = defaultNS;
    io:println("default-namespace ok"); // @output default-namespace ok

    var defaultViaPrefix = testXMLNavigationOnSingleElementReferToDefaultNSViaPrefix();
    _ = defaultViaPrefix;
    io:println("default-namespace-prefix ok"); // @output default-namespace-prefix ok

    var sequence = testXMLNavigationOnSequence();
    assert(sequence[0], xml `<child>A</child><child>B</child><child>C</child>`);
    assert(sequence[1], xml `<child>A</child><child>B</child><child>C</child><it-child>D</it-child>TEXT`);
    assert(sequence[2], xml `<child>A</child><child>B</child><child>C</child><it-child>D</it-child>`);
    assert(sequence[3], xml `<child>A</child><child>B</child><child>C</child>`);
    assert(sequence[4], xml `<child>A</child><child>B</child><child>C</child>`);
    io:println("sequence ok"); // @output sequence ok

    var sequenceNS = testXMLNavigationOnSequenceWithNamespaces();
    _ = sequenceNS;
    io:println("namespaced-sequence ok"); // @output namespaced-sequence ok

    var multiple = testXMLNavigationOnSequenceWithNamespacesAndMultipleFilters();
    _ = multiple;
    io:println("multiple-filters ok"); // @output multiple-filters ok

    var composition = testXMLElementAccessNavigationAccessComposition();
    _ = composition;
    io:println("composition ok"); // @output composition ok

    var quoted = testXMLNavigationExpressionWithQuotedIdentifiers();
    _ = quoted;
    io:println("quoted-identifiers ok"); // @output quoted-identifiers ok

    testXMLNavigationExpressionWithXMLSubtypeOnLHS();
    io:println("subtype-lhs ok"); // @output subtype-lhs ok
    testXMLNavigationDescendantsStepWithXMLSubtypeOnLHS();
    io:println("descendant-subtype-lhs ok"); // @output descendant-subtype-lhs ok
    testXMLNavigationWithEscapeCharacter();
    io:println("escape-character ok"); // @output escape-character ok
    testXmlNavigationWithUnionType();
    io:println("xml-union ok"); // @output xml-union ok
    testXmlNavigationWithDefaultNamespaceDefinedAfter();
    io:println("namespace-after ok"); // @output namespace-after ok
    testXmlIndexedStepExtend();
    io:println("indexed-extension ok"); // @output indexed-extension ok
    testXmlFilterStepExtend();
    io:println("filter-extension ok"); // @output filter-extension ok
    testXmlIndexedAndFilterStepExtend();
    io:println("mixed-index-filter ok"); // @output mixed-index-filter ok
    check testXmlMethodCallStepExtend();
    io:println("method-extension ok"); // @output method-extension ok
    testXmlMethodCallIndexedAndFilterStepExtend();
    io:println("mixed-method-extension ok"); // @output mixed-method-extension ok
}
