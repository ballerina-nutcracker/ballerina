...
public function main() {
    ...
    if value is WithY {
        var narrowed = {x: 1, ...value}; // @error future: spread of a narrowed mapping type
    ...
    if !(mixed is ReadonlyInt) {
        ...
        record {|string m;|} narrowedMember = {...mixed}; // @error future: spread of a narrowed mapping type
        io:println(narrowedMember); // @output {"m":"a"}
    }
    WithX|WithY other = {x: 2};
    if !(other is WithY) {
        var narrowed = {...other, y: "b"}; // @error future: spread of a narrowed mapping type
